...
function helpEmbed() {
  ...
  return surface({
    ...
    description: [
      ...
      "",
      "**Setup**",
      `\`${p}serversetup\` / \`/serversetup\` (alias \`${p}tsbsetup\`) — leaderboard, ranking, score, lineups, tryouts`,
      "",
      "**Profile & coach**",
      `\`/profile\` · \`${p}profile\``,
      ...
      `\`${p}lineup\` / \`/lineup\` — add · replace · remove · sub · publish`,
      `\`${p}stage\` / \`/stage\` (aliases: tier, rank, phase) — assigns Discord roles`,
      `\`${p}score\` / \`/score\``,
